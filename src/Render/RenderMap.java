...
public class RenderMap {
					...
					if (!boolList.get(i)) {
						...
						if (item != null) {
							...
							if (item.getName().endsWith(".wdr")) {
								...
								mdl = new Model().loadWDR(br, item.getSize(), gl); // load
																					// the
																					// model
																					// from
																					// img
							} else if (item.getName().endsWith(".wdd")) {
								mdl = new Model().loadWDD(br, item.getSize(), gl, ideList.get(i).modelName);
							...
							if (item != null) {
								...
								TextureDic txd = new TextureDic(texName, br, gameType, gl, item.getSize());
	...
	private void loadAddedModel(GL2 gl) {
		...
		if (item != null) {
			...
			if (item.getName().endsWith(".wdr")) {
				System.out.println(item.getName());
				mdl = new Model().loadWDR(br, item.getSize(), gl); // load the
																	// model
																	// from img
			} else if (item.getName().endsWith(".wdd")) {
				mdl = new Model().loadWDD(br, item.getSize(), gl, tempIDE.modelName);
			} else if (item.getName().endsWith(".wft")) {
				System.out.println("Loading WFT: " + item.getName());
				mdl = new Model().loadWFT(br, item.getSize(), gl);
			}
			...
			if (item != null) {
				rf.seek(item.getOffset());
				br = rf.getByteReader(item.getSize());
				TextureDic txd = new TextureDic(tempIDE.textureName + ".wtd", br, gameType, gl, item.getSize());
