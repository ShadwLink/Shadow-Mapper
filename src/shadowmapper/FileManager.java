...
public class FileManager extends Thread{
    ...
    public void addIMG(String file){
        IMG tempIMG = new IMG(file, Finals.gIV, null, false, true);
    ...
    public void init(){
        ...
        for(int i = 0; i < gta_dat.img.size(); i++){
            ...
            line = line + ".img";
            imgs[i] = new IMG(line, Finals.gIV, key, true, containsProps);
            lb.addOneToLoadingBar();
        }
        ...
        for(int i = 0; i < imgs.length; i++){
            if(imgs[i].wplCount > 0){
                ReadFunctions rf = new ReadFunctions(); //open the img file
                if(rf.openFile(imgs[i].getFileName())){
                    for(int j = 0; j < imgs[i].Items.size(); j++){
                        if(imgs[i].Items.get(j).getName().toLowerCase().endsWith(".wpl")){
                            rf.seek(imgs[i].Items.get(j).getOffset());
                            IPL tempIPL = new IPL(rf, Finals.gIV, true, imgs[i], imgs[i].Items.get(j));
                            tempIPL.setFileName(imgs[i].Items.get(j).getName());
                            lb.setLabelText("<WPL> " + imgs[i].Items.get(j).getName());
                            iplList.add(tempIPL);
                            modelIPL.addElement(imgs[i].Items.get(j).getName());
                            lb.addOneToLoadingBar();
                        }
                    }
                }
