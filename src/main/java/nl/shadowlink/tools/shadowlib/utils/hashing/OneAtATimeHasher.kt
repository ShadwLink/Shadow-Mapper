package nl.shadowlink.tools.shadowlib.utils.hashing

class OneAtATimeHasher : Hasher {

    override fun hash(value: String): Long {
        return getHashKey(value)
    }

    companion object {

        private val lookUpTable = arrayOf(
            ,
        )

        private fun getHashKeySubString(str: String, initialHash: UInt): UInt {
            var hash = initialHash
            str.chars().forEach { c ->
                hash += lookUpTable[c].toUInt()
                hash += (hash shl 10)
                hash = hash xor (hash.toLong() ushr 6).toUInt()
            }
            return hash
        }

        private fun getHashKeyFinalize(str: String, initialHash: UInt): UInt {
            var hash = getHashKeySubString(str, initialHash)
            hash += (hash shl 3)
            hash = hash xor (hash.toLong() ushr 11).toUInt()
            hash += (hash shl 15)
            return hash
        }

        @JvmStatic
        fun getHashKey(str: String, initialHash: UInt = 0u): Long {
            return getHashKeyFinalize(str, initialHash).toLong()
        }

        @JvmStatic
        fun getHashKey(str: String): Long {
            return getHashKeyFinalize(str, 0u).toLong()
        }
    }
}